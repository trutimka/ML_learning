...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        num_train = self.X_train.shape[0]
        dists = np.zeros((num_test, num_train))
        test_points_expanded = np.sum(X ** 2, axis=1, keepdims=True)
        train_points_expanded = np.sum(self.X_train ** 2, axis=1)
        inner_product = X @ self.X_train.T
        dists = np.sqrt(test_points_expanded + train_points_expanded - 2 * inner_product)
        return dists
